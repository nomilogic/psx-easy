...
export default function GlobalHeaderTicker() {
  ...
  useEffect(() => {
    if (lastMessage && lastMessage.type === 'market_update' && lastMessage.data) {
      if (lastMessage.data.stocks) {
        setStocks(lastMessage.data.stocks);
      }
    }
  }, [lastMessage]);
  ...
  // Get top 15 stocks by volume for ticker
  ...
  if (!tickerStocks.length) {
    return (
      <div className="bg-slate-900 text-white py-2 border-b fixed top-0 left-0 right-0 z-50">
